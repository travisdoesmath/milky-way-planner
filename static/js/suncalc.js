...
SunCalc.getGalacticCenterTimes = function (date, lat, lng) {
    var t = new Date(date);

    var h0 = SunCalc.getGCPosition(t, lat, lng).altitude,
        h1, h2, rise, set, a, b, xe, ye, d, roots, x1, x2, dx;

    // go in 2-hour chunks, each time seeing if a 3-point quadratic curve crosses zero (which means rise or set)
    for (var i = 1; i <= 24; i += 2) {
        h1 = SunCalc.getGCPosition(hoursLater(t, i), lat, lng).altitude;
        h2 = SunCalc.getGCPosition(hoursLater(t, i + 1), lat, lng).altitude;
    ...
    if (set) result.set = hoursLater(t, set);
    if (rise && set) {
        result.transit = hoursLater(t, 0.5 * (rise + set));
        // result.maxAltitude = SunCalc.getGCPosition(result.transit, lat, lng).altitude / rad;
    }
